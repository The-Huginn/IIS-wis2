...
const CourseManager = () => {
    ...
    const authContext = useContext(ErrorContext)
    ...
    useEffect(() => {
        if (value)
        {
            value.getAllCourses().then(courses => setCourses(courses)).catch(() => authContext.setValue(true)).finally(() => setLoading(false)).finally(() => setLoaded(true))
        }
    }, [])
